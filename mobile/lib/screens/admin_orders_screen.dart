...
class _AdminOrdersScreenState extends State<AdminOrdersScreen>
  ...
  Future<void> _approve(Map order) async {
    final res = await ApiService.patch2(
        '/orders/${order['id']}/status', {'status': 'CONFIRMED'});
    if (!mounted) return;
    if (res.statusCode == 200) {
  ...
  Future<void> _decline(Map order) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
            'Decline order from ${order['user']?['fullName'] ?? order['user']?['username']}?',
    ...
    if (!ok) return;
    final res = await ApiService.patch2(
        '/orders/${order['id']}/status', {'status': 'DECLINED'});
    if (!mounted) return;
    if (res.statusCode == 200) {
      showSnack(context, 'Order declined');
      _load();
    }
  }
  ...
  Widget _orderCard(Map o, {required String type}) {
    ...
    final fullProofUrl = proofUrl.startsWith('/api')
        ...
        : proofUrl;
    final isCash =
        (o['paymentMethod'] ?? '').toString().toLowerCase().contains('cash');
            ...
            if (isCash && type == 'submitted') ...[
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Text('Customer will pay cash in person.',
                        style: GoogleFonts.outfit(
